...
import static com.example.android.sunshine.utilities.SunshineDateUtils.LOG;
...
public class SunshineRepository {
    ...
    private SunshineRepository(WeatherDao weatherDao,
        ...
        //Observe data with your LiveData
        netWorkData.observeForever(newForecastsFromNetwork -> {
            mExecutors.diskIO().execute(()-> {
                // Inserir nossos novos dados do tempo no banco de dados do Sunshine
                deleteOldData();
                Log.d(LOG_TAG, "Old values are deleted");
    ...
    public LiveData<List<ListViewWeatherEntry>> getCurrentWeatherForecasts(Date date){
        ...
        return mWeatherDao.getAListOfWheaterEntry(date);
    }
    ...
    private boolean isFetchNeeded() {
        Log.d(LOG, "future days in the bank: " + mWeatherDao.countAllFutureWeather(SunshineDateUtils.getNormalizedUtcDateForToday()));
        Date today = SunshineDateUtils.getNormalizedUtcDateForToday();
        int datesSavedInTheBankForTodayAnAfter = mWeatherDao.countAllFutureWeather(today);
        int numberOfDaysIWantToSave = WeatherNetworkDataSource.NUM_DAYS;
        return numberOfDaysIWantToSave > datesSavedInTheBankForTodayAnAfter;
    }
