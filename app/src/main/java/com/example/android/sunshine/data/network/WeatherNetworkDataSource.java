...
public class WeatherNetworkDataSource {
    ...
    // The tutorial needed is create that class
    private final MutableLiveData<WeatherEntry[]> mDownloadedWeatherForecasts;
    private SunshineRepository repository;

    private WeatherNetworkDataSource(Context context, AppExecutors executors, SunshineRepository repository) {
        mContext = context;
    ...
    public static WeatherNetworkDataSource getInstance(Context context, AppExecutors executors, SunshineRepository repository) {
        Log.d(LOG_TAG, "Getting the network data source");
        if (sInstance == null) {
            synchronized (LOCK) {
                sInstance = new WeatherNetworkDataSource(context.getApplicationContext(), executors, repository);
                Log.d(LOG_TAG, "Made new network data source");
            }
        }
        return sInstance;
